...
<h1><?php echo htmlready($groupname)?></h1>
<h2><?php echo htmlready($weekday)?>
<?php 
if ($only_one_week) {
    echo ' ' . strftime('%x', $current_day);
}?>
</h2>
<h2>
<?php 
...
<h3><?php echo "Raumanzahl: " . count($data)?></h3>
<?php
...
if (!$only_one_week) {
    ...
    foreach ($data as $room) {
        $room_data = array_filter($room['room_data'], create_function('$a', 'return isset($a["metadate_id"]);'));
        if (!count($room_data)) continue;
        ?>
        <tr>
        <td colspan="5">
        <span style="font-weight:bold;"><?php echo htmlready($room['name'])?></span>
        </td>
        </tr>
        <?php 
        foreach ($room_data  as $one) { ?>
        <tr>
        <td width="10%">
